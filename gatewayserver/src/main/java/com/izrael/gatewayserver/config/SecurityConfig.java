...
public class SecurityConfig {
    ...
    public SecurityWebFilterChain springSecurityFilterChain(ServerHttpSecurity http) {
        http.authorizeExchange(exchanges -> exchanges.pathMatchers("/applications/apihotel/**").hasRole("HOTELS")
        		.pathMatchers("/applications/apirooms/**").authenticated()
        		.pathMatchers("/applications/apireservations/**").permitAll())
        	.oauth2ResourceServer().jwt().jwtAuthenticationConverter(grantedAuthorities());
        http.csrf().disable();
        return http.build();
    }
